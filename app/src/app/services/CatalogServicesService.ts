...
/** Lightweight representation of a ServiceCatalogService entity. */
...
export class CatalogServicesService {
  ...
  readonly items = signal<CatalogServiceItem[]>([]);
  ...
  /** Fast lookup by id. */
  private idMap = signal<Map<string, CatalogServiceItem>>(new Map());
  ...
  private load(): void {
    ...
    this.entityApi.listAllServiceCatalogServices().subscribe({
      next: (list) => {
        if (seq !== this.loadSeq) return;
        const safeList = Array.isArray(list) ? list : [];
        const items: CatalogServiceItem[] = safeList.map((e) => ({
        ...
        const map = new Map<string, CatalogServiceItem>();
  ...
  getById(id: string): CatalogServiceItem | undefined {
