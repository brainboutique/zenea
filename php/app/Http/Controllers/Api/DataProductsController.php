...
class DataProductsController extends Controller
{
    ...
    /**
     * @OA\Get(
     *     path="/api/v1/{repoName}/{branch}/data-products",
     *     operationId="getDataProductsRepoBranch",
     *     tags={"DataProducts"},
     *     summary="Get data products document",
     *     description="Returns the list of active data products from /data/{repoName}/{branch}. Use repoName=local, branch=default for default data.",
     ...
     *     @OA\Response(response="200", description="Data products document", @OA\JsonContent()),
     * )
     */
    public function getDataProducts(string $repoName, string $branch): JsonResponse
    {
        $path = $this->resolvePath($repoName, $branch);
        $data = $this->entityService->getCached('DataProduct', $path);
