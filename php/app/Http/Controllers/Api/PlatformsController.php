...
class PlatformsController extends Controller
{
    ...
    /**
     * @OA\Get(
     *     path="/api/v1/{repoName}/{branch}/platforms",
     *     operationId="getPlatformsRepoBranch",
     *     tags={"Platforms"},
     *     summary="Get platforms document",
     *     description="Returns the list of active platforms from /data/{repoName}/{branch}. Use repoName=local, branch=default for default data.",
     ...
     *     @OA\Response(response="200", description="Platforms document", @OA\JsonContent()),
     * )
     */
    public function getPlatforms(string $repoName, string $branch): JsonResponse
    {
        $path = $this->resolvePath($repoName, $branch);
        $data = $this->entityService->getCached('Platform', $path);
